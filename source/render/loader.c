...
/*   Updated: 2023/10/18 14:13:24 by nklingsh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	data->tex->wall_no.mlx_img = mlx_xpm_file_to_image(\
	data->mlx_ptr, data->map->north, \
	...
	&data->tex->wall_no.line_len, &data->tex->wall_no.endian);
	return (0);
}

int	load_south(t_data *data)
{
	data->tex->wall_so.mlx_img = mlx_xpm_file_to_image(\
	data->mlx_ptr, data->map->south, \
	...
	&data->tex->wall_so.bpp, \
	&data->tex->wall_so.line_len, &data->tex->wall_so.endian);
	return (0);
}

int	load_west(t_data *data)
{
	data->tex->wall_we.mlx_img = mlx_xpm_file_to_image(\
	data->mlx_ptr, data->map->west, \
	...
	&data->tex->wall_we.bpp, \
	&data->tex->wall_we.line_len, &data->tex->wall_we.endian);
	return (0);
}

int	load_east(t_data *data)
{
	data->tex->wall_ea.mlx_img = mlx_xpm_file_to_image(\
	data->mlx_ptr, data->map->east, \
	&data->tex->wall_ea.tex_width, &data->tex->wall_ea.tex_height);
	if (!data->tex->wall_ea.mlx_img)
		return (-2);
	data->tex->wall_ea.addr = mlx_get_data_addr(\
	data->tex->wall_ea.mlx_img, \
	&data->tex->wall_ea.bpp, \
